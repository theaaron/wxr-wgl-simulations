...
// cut plane state — normalized [0,1] voxel space; 1.0 = no cut
...
function drawVoxels(projMatrix, viewMatrix, modelMatrix) {
    ...
    gl.uniform1f(gl.getUniformLocation(peelProg, 'u_cutX'), cutX);
    gl.uniform1f(gl.getUniformLocation(peelProg, 'u_cutY'), cutY);
    gl.uniform1f(gl.getUniformLocation(peelProg, 'u_cutZ'), cutZ);
        ...
        if (!structure) {
            ...
            try {
                ...
                window.addEventListener('keydown', e => {
                    ...
                    if (e.code === 'Space') { e.preventDefault(); simRunning = !simRunning; if (simRunning) paceAt(kx, ky, kz, 12); }
                    else if (e.code === 'KeyP') paceAt(kx, ky, kz, 12);
